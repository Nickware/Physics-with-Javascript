// This code defines a Graph class that can be used to create and manipulate a graph on an HTML canvas. The class includes methods for drawing a grid, axes, and plotting data points. The constructor takes parameters for the canvas context, the minimum and maximum values for the x and y axes, the location of the origin, and the width and height of the graph in pixels. The drawgrid method allows you to specify the major and minor tick intervals for both axes, while the drawaxes method allows you to label the axes. The plot method takes arrays of x and y values, along with optional parameters for color, whether to plot dots, and whether to connect the points with lines. This class provides a flexible way to visualize data on a canvas element.
...
// VARIABLE DECLARATIONS	
	...
	var x_displ_scal;
	var y_displ_scal;
	// width and height of the textbox used for displaying values on the axes
